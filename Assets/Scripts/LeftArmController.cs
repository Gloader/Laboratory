...
[System.Serializable]
public class LeftArmBendingSegment {
...
[System.Serializable]
public class LeftArmNonAffectedJoints {
...
public class LeftArmController : MonoBehaviour {
    ...
    public LeftArmBendingSegment[] segments;
    public LeftArmNonAffectedJoints[] LeftArmNonAffectedJoints;
    ...
    void Start () {
        ...
        // Setup segments
        foreach (LeftArmBendingSegment segment in segments) {
    ...
    void LateUpdate () {
        ...
        Vector3[] jointDirections = new Vector3[LeftArmNonAffectedJoints.Length];
        for (int i=0; i<LeftArmNonAffectedJoints.Length; i++) {
            foreach (Transform child in LeftArmNonAffectedJoints[i].joint) {
                jointDirections[i] = child.position - LeftArmNonAffectedJoints[i].joint.position;
                break;
            }
        }
        ...
        for (int i=0; i<LeftArmNonAffectedJoints.Length; i++) {
            ...
            foreach (Transform child in LeftArmNonAffectedJoints[i].joint) {
                newJointDirection = child.position - LeftArmNonAffectedJoints[i].joint.position;
                break;
            }
            
            Vector3 combinedJointDirection = Vector3.Slerp(
                jointDirections[i], newJointDirection, LeftArmNonAffectedJoints[i].effect
                );
            
            LeftArmNonAffectedJoints[i].joint.rotation = Quaternion.FromToRotation(
                newJointDirection, combinedJointDirection
                ) * LeftArmNonAffectedJoints[i].joint.rotation;
        }
    }
