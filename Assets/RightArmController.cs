...
[System.Serializable]
public class RightArmBendingSegment {
...
[System.Serializable]
public class RightArmNonAffectedJoints {
...
public class RightArmController : MonoBehaviour {
    ...
    public RightArmBendingSegment[] segments;
    public RightArmNonAffectedJoints[] RightArmNonAffectedJoints;
    ...
    void Start () {
        ...
        // Setup segments
        foreach (RightArmBendingSegment segment in segments) {
    ...
    void LateUpdate () {
        ...
        Vector3[] jointDirections = new Vector3[RightArmNonAffectedJoints.Length];
        for (int i=0; i<RightArmNonAffectedJoints.Length; i++) {
            foreach (Transform child in RightArmNonAffectedJoints[i].joint) {
                jointDirections[i] = child.position - RightArmNonAffectedJoints[i].joint.position;
                break;
            }
        }
        ...
        for (int i=0; i<RightArmNonAffectedJoints.Length; i++) {
            ...
            foreach (Transform child in RightArmNonAffectedJoints[i].joint) {
                newJointDirection = child.position - RightArmNonAffectedJoints[i].joint.position;
                break;
            }
            
            Vector3 combinedJointDirection = Vector3.Slerp(
                jointDirections[i], newJointDirection, RightArmNonAffectedJoints[i].effect
                );
            
            RightArmNonAffectedJoints[i].joint.rotation = Quaternion.FromToRotation(
                newJointDirection, combinedJointDirection
                ) * RightArmNonAffectedJoints[i].joint.rotation;
        }
    }
